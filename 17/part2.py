...
class Tower:
    ...
    def get_sim_id(self):
        return f'{self.rock_source.next_rock_index}_{self.wind_source.next_wind_direction_index}_{"".join([str(x) for x in self.tower[self.rock_height]])}'
...
def main():
    ...
    for i in range(total_rocks):
        ...
        sim_id = tower.get_sim_id()
        rock_height[sim_id] = rock_height.get(sim_id, [])+[tower.rock_height]
        rock_seq[sim_id] = rock_seq.get(sim_id, [])+[i+1]

        if len(rock_height[sim_id]) < 3:
            continue
        if len(rock_seq[sim_id]) < 3:
            continue

        h1, h2, h3 = rock_height[sim_id][-3:]
        s1, s2, s3 = rock_seq[sim_id][-3:]

        if h3 - h2 == h2 - h1 and s3 - s2 == s2 - s1:

            multiplier, remainder = divmod(total_rocks - (i+1), s3 - s2)
            total_height = multiplier*(h3 - h2)
            for _ in range(remainder):
                tower.add_rock()
            total_height += tower.rock_height
            print(total_height)
            break
