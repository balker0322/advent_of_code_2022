...
def get_min_distance(g, starting_node, end_coor):
        ...
        for neighbor_node in g[current_node]:
            ...
            if (row, col) == end_coor:
                if shortest_path[neighbor_node] < mininum_distance:
                    mininum_distance = shortest_path[neighbor_node]
    ...
    return mininum_distance, min_node
...
def main():
    ...
    map.add_map_borders()
    start_coor = (0, 1)
    start_tile = Tile('.', start_coor[0], start_coor[1])
    end_coor = (map.row_count+1, map.row_count)
    end_tile = Tile('.', end_coor[0], end_coor[1])
    map.add_start_tile(start_tile)
    map.add_end_tile(end_tile)

    g = CoorStateGraph(map)

    starting_node = (start_coor[0],start_coor[1],0)
    graph = g.get_graph()
    total = 0
    shortest_distance, next_node = get_min_distance(graph, starting_node, end_tile.get_key())
    print(shortest_distance)
    total += shortest_distance
    shortest_distance, next_node = get_min_distance(graph, next_node, start_tile.get_key())
    print(shortest_distance)
    total += shortest_distance
    shortest_distance, next_node = get_min_distance(graph, next_node, end_tile.get_key())
    print(shortest_distance)
    total += shortest_distance
    print(total)
